...
export function handleExpectedDateChange(
  pedidoCompra,
  infoPedido,
  infoProdComprados
) {
    ...
    if (
      ...
      if (line !== "") {
        switch (lineNumber) {
          ...
          case 23:
            ...
            break;
          case 2:
            infoPedido.lote = line;
            break;
          case 16:
            infoPedido.valor = line;
            break;
        }
      }
    } else if (!isValoresExpressos) {
      const adjustedLineNumber = hasFernandez
        ? lineNumber - 54
        : lineNumber - 56;
      if (adjustedLineNumber % 19 === 0) {
        ...
        if (Object.keys(prodComprado).length !== 0) {
          infoProdComprados.push(
            renameProperties(removeEmptyProperties(prodComprado))
          );
          prodComprado = {};
        }
      }
...
export function handleProdCompradoLine(
  line,
  prodComprado,
  lineNumber,
  hasFernandez
...
export function renameProperties(obj) {
  ...
  newObj["status"] = "COMPRADO"; // Inicialmente como COMPRADO
  return newObj;
}
