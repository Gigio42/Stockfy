...
result.forEach(obj => fetchChapas(obj.id_compra)); // Criar uma linha para cada objeto
result.forEach(obj => criarTable(table, obj)); // Criar uma linha para cada objeto

} else if (text.includes('PENHA')) {
var result = penha(text); // result é um array de objetos
const table = document.getElementById('recebimento').getElementsByTagName('tbody')[0];
result.forEach(obj => fetchChapas(obj.id_compra));
result.forEach(obj => criarTable(table, obj)); // Criar uma linha para cada objeto
...
function organizarirani(array) {
    var indexesToRemove = [1, 2, 3, 6, 8, 9, 10, 11, 12, 13, 18, 19, 20, 21, 22];
    var newArray = [];
    console.log(array)
...
function fernandez(fullText) {
    ...
    return organizarFernandez(resultArray);
}

function organizarFernandez(array) {
    var indexesToRemove = [2, 3, 6, 8, 9, 10, 11];
    var newArray = [];
...
function parseXML(xml) {
    ...
    Array.from(products).forEach(product => {
        ...
        if (!prodDetails) return; // Early exit if no valid supplier or function found

        const chapaData = data_Chapa(prodDetails, product, supplier, xmlDoc);
        var id_compra = chapaData.id_compra
        fetchChapas(id_compra)
        criarTable(table, chapaData);
    });
}

function extractSupplier(xmlDoc) {
    const xNome = xmlDoc.getElementsByTagName("emit")[0].getElementsByTagName("xNome")[0].textContent;
    return ["PENHA", "FERNANDEZ", "Irani"].find(s => xNome.includes(s)) || "";
}

    
function data_Chapa(prodDetails, product, supplier, xmlDoc) {
    const xPed = product.getElementsByTagName("xPed")[0].textContent.replace(/\D/g, '');
    ...
    return {
        ...
        vincos: vincada,
        valor_unitario: vUnCom // Valor agora sempre será um número
    };
}
